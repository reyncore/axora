  ...
  callbacks: {
    jwt({ token, user }) {
      ...
      if (user) {
        ...
        // Simpan timestamp passwordChangedAt saat token pertama diterbitkan.
        // Nilai ini akan dibandingkan dengan DB di middleware untuk invalidasi.
        token.issuedAtPassword = Date.now()
      }
      return token
    },
