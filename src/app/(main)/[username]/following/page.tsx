...
import { fetchFollowingPage } from "@/lib/follow-list"
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title:  `Following — ${user.displayName} (@${username})`,
...
export default async function FollowingPage({ params }: Props) {
  ...
  const result = await fetchFollowingPage(username, session?.user?.id)
  if (!result) notFound()
  ...
  return (
    <div>
      ...
      {/* Tab navigation — link ke followers, aktif di following */}
      ...
      <UserList
        ...
        emptyMessage={`@${username} belum mengikuti siapapun`}
      />
    </div>
  )
}
