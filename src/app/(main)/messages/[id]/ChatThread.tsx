...
import { Send, Loader2, Check, CheckCheck, Paperclip, X } from "lucide-react"
...
export function ChatThread({ conversationId, initialMessages, currentUserId }: Props) {
  ...
  useEffect(() => {
    ...
    es.addEventListener("message", (e) => {
      try {
        ...
        if (payload.conversationId === conversationId) void fetchLatest()
      } catch { /* ignore */ }
    })
    es.onerror = () => es.close()
    return () => es.close()
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/conversations/${conversationId}/messages`, {
        ...
        body:    JSON.stringify({
          content:  trimmed || (attachment ? `[Lampiran: ${attachment.fileName}]` : ""),
  ...
  const canSend     = (content.trim().length > 0 || !!attachment) && !isOverLimit && !sending
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      {/* Upload area — muncul saat tombol attachment diklik */}
      {showUpload && (
        <div className="border-t border-ax-bg-border px-3 pt-3">
          {attachment ? (
            <div className="flex items-center gap-3 p-2 bg-ax-bg-elevated rounded-xl
                            border border-ax-bg-border">
              {attachment.type === "IMAGE" || attachment.type === "GIF" ? (
                <img
                  src={attachment.thumbUrl ?? attachment.fileUrl}
                  alt={attachment.fileName}
                  className="w-12 h-12 object-cover rounded-lg flex-shrink-0"
                />
              ) : (
                <div className="w-12 h-12 bg-ax-bg-subtle rounded-lg flex items-center
                                justify-center flex-shrink-0 text-ax-text-muted">
                  🎬
                </div>
              )}
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-ax-text-primary truncate">
                  {attachment.fileName}
                </p>
                <p className="text-xs text-ax-text-muted">Siap dikirim</p>
              </div>
              <button
                type="button"
                onClick={() => { setAttachment(null); setShowUpload(false) }}
                className="p-1 text-ax-text-muted hover:text-red-400 transition-colors"
      ...
      <div className="border-t border-ax-bg-border p-3">
        <form onSubmit={handleSend} className="flex items-end gap-2">
          ...
          <button
            ...
            onClick={() => {
              setShowUpload(v => !v)
              if (showUpload) setAttachment(null)
            }}
            className={cn(
              "w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0",
              "transition-colors",
              showUpload || attachment
                ? "bg-ax-accent-muted text-ax-accent-light"
                : "text-ax-text-muted hover:text-ax-text-secondary hover:bg-ax-bg-subtle"
            )}
          ...
          <div className="flex-1 bg-ax-bg-elevated border border-ax-bg-border rounded-2xl
            ...
            <textarea
              ...
              placeholder={attachment ? "Tambah caption (opsional)..." : "Tulis pesan..."}
              rows={1}
